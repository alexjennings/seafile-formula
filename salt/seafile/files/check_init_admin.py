...
ANSWER_FILE = "/opt/seafile/seafile-server-latest/seafile_answers.txt"

parser = ConfigParser()
with open(ANSWER_FILE) as stream:
    fakefile = StringIO("[seafile]\n" + stream.read())
...
def main():
    ...
    email = parser.get('seafile', 'seahub_admin_email')
    # Might have quotes around email address in config file
    if email.startswith('"') and email.endswith('"'):
        email = email[1:-1]
    passwd = parser.get('seafile', 'seahub_admin_passwd')
